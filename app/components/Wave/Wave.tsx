...
export default function Wave({ katongData }: WaveProps) {
    ...
    useEffect(() => {
        const getRandomKatongs = () => {
            ...
            return katongData.sort(() => 0.5 - Math.random()).slice(0, numberOfKatongs)
        ...
        const interval = setInterval(() => {
            setDisplayData(currentDisplay => {
                const fadingDisplay = currentDisplay.map(item => ({ ...item, styles: { ...item.styles, opacity: 0 } }));
...
>
  {item.wish}
                    ...
                    <div className="katong-image-container">
                        <Image src={katongImages[item.katong - 1]} alt="Katong" width={100} height={100} />
                    </div>
                </div>
            </div>
        ))}
